...
void Drive::arcade_curvatherp_flipped(e_type stick_type,
  ...
  if (stick_type == SPLIT) {
    // Put the joysticks through the curve function
    fwd_stick = right_curve_function(
        master.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_Y));
    turn_stick = left_curve_function(
        master.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_X));
  } else if (stick_type == SINGLE) {
    // Put the joysticks through the curve function
    fwd_stick = right_curve_function(
...
void Drive::curvatherp(int forward_stick, int turn_stick, double interpolator) {
  ...
  double left_curvature =
      forward_stick_clamped + fabs(forward_stick_clamped) * turn_stick_clamped;
  ...
  double left_tank =
      util::clip_num(forward_stick_clamped + turn_stick_clamped, 1.0, -1.0);
  double right_tank =
      util::clip_num(forward_stick_clamped - turn_stick_clamped, 1.0, -1.0);

  // interpolate so that at high speeds (high interpolator values) rely more on
  // cheezy and low speeds rely more on tank
  double left_speed =
